...
const AbstractType1 = () => {
    ...
    const handleConfirmBody = () => {
        ...
        } else if (form.getFieldValue('palavrasChave') === undefined) {
            notification.error({
                message: 'Erro',
                description: 'Por favor insira as palavras chave.'
            })
        } else {
            setCurrentStep(2);
        }
    ...
    const submit = () => {
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/api/resume/`, data, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
            ...
            <div className="flex flex-col gap-2 bg-white rounded-lg p-4">
                ...
                {currentStep === 1 && (
                    <div className='w-full'>
                        ...
                        >
                            Possui {countWords(body.introducao) + countWords(body.metodos) + countWords(body.resultados) + countWords(body.conclusao)}/350 Palavra(s) para preencher o corpo do resumo</p>
                        <Form
                            form={form}
                            layout='vertical'
                            onFinish={null}
