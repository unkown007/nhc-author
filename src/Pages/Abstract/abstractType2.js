...
const AbstractType2 = () => {
    ...
    const handleConfirmBody = () => {
        ...
        } else if (form.getFieldValue('resumo') === undefined) {
            notification.error({
                message: 'Erro',
                description: 'Por favor insira o Resumo do caso.'
            })
        } else if (form.getFieldValue('discussao') === undefined) {
            notification.error({
                message: 'Erro',
                description: 'Por favor insira a Discussão.'
            })
        } else if (form.getFieldValue('conclusao') === undefined) {
            notification.error({
                message: 'Erro',
                description: 'Por favor insira a conclusão.'
            })
        } else {
            setCurrentStep(2);
        }
    ...
    const submit = () => {
        ...
        let data = {
            ...
            ethic: ethic,
            resume: form.getFieldValue('resumo'),
            discussion: form.getFieldValue('discussao'),
            conclusion: form.getFieldValue('conclusao'),
            keyword: keywords,
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/api/resume/clinical/`, data, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
            ...
            <div className="flex flex-col gap-2 bg-white rounded-lg p-4">
                ...
                {currentStep === 1 && (
                    <div className='w-full'>
                        ...
                        >
                            Possui {countWords(body.introducao) + countWords(body.resumo) + countWords(body.discussao) + countWords(body.conclusao)}/350 Palavra(s) para preencher o corpo do resumo</p>
                        <Form
                            form={form}
                            layout='vertical'
                            onFinish={null}
                            ...
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                        rules={[
                                            { required: true, message: 'Por favor insira o resumo' },
                                        ]}
                                    >
                                        <Input.TextArea placeholder='Insira o Resumo do caso (identificação, resumo da história clínica, exames complementares e/ou intervenções, diagnóstico final)'
                                                        value={body.resumo}
                                                        onChange={(e) => {
                                                            setBody({ ...body, resumo: e.target.value })
                                                            form.setFieldsValue({ resumo: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(body.resumo)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                        rules={[
                                            { required: true, message: 'Por favor insira a discussão' },
                                        ]}
                                    >
                                        <Input.TextArea placeholder='Insira a Discussão'
                                                        value={body.discussao}
                                                        onChange={(e) => {
                                                            setBody({ ...body, discussao: e.target.value })
                                                            form.setFieldsValue({ discussao: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(body.discussao)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                        rules={[
                                            { required: true, message: 'Por favor insira a conclusão' },
                                        ]}
                                    >
                                        <Input.TextArea placeholder='Insira a conclusão'
                                                        value={body.conclusao}
                                                        onChange={(e) => {
                                                            setBody({ ...body, conclusao: e.target.value })
                                                            form.setFieldsValue({ conclusao: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(body.conclusao)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
                                ...
                                <div className='w-full bg-white rounded-lg p-4'>
                                    <div className='w-full flex flex-col gap-4'>
                                        ...
                                        <div className='w-full'>
                                            ...
                                            <p className='text-sm'>{body.resumo}</p>
                                        </div>
                                        <div className='w-full'>
                                            <p className='text-sm font-bold mb-2'>Discussão</p>
                                            <p className='text-sm'>{body.discussao}</p>
                                        </div>
                                        <div className='w-full'>
                                            <p className='text-sm font-bold mb-2'>Conclusão</p>
                                            <p className='text-sm'>{body.conclusao}</p>
                                        </div>
                                        <div className='w-full'>
