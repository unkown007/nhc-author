...
const ListAbstractType1 = () => {
    ...
    const getResumes = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/user/${JSON.parse(sessionStorage.getItem('user')).user}/author/resume/`, {
                headers: {
                    Authorization: `Token ${sessionStorage.getItem('token')}`
                }
            });
    ...
    const handleRemoveReview = (record) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/api/resume/${record.id}/`, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
    ...
    const submit = () => {
        ...
        let data = {
            ...
            // objective: form.getFieldValue('objetivos'),
            ethic: selected.ethic,
            methodology: selected.metodos, // form.getFieldValue('metodos'),
            result: selected.resultados, // form.getFieldValue('resultados'),
        ...
        axios.put(`${process.env.REACT_APP_API_URL}/api/resume/${selected.id}/`, data, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
                    ...
                    <div className='w-full'>
                        <div className='w-full flex flex-row justify-between items-center mb-0'>
                            ...
                            <div className='hidden'>
                                <ComponentToPrint ref={printRef}>
                                    <Modelo abstract={selected} /> {/* passa a linha selecionada como prop */}
                                </ComponentToPrint>
                            </div>
                        </div>
                        <div className='w-full flex flex-col gap-2'>
                            ...
                            <div className='w-full'>
                                ...
                                <p className='text-sm rounded-md' style={{border: 'none'}}>{selected?.keyword?.map((item, index) => <Tag
                            ...
                            <div className='w-full'>
                                ...
                                {selected.reviews?.map((item, index) => (
                                    <div key={index} className='w-full flex flex-col gap-2'>
                                        <p className='text-sm mb-2 flex items-center'>
                                            <FaRegComment className='mr-2'/>
                                            : {item?.status?.name} •  <MdOutlineGrade className='mr-2' style={{ width: '22px', height: '22px' }}/>
...
export default ListAbstractType1;