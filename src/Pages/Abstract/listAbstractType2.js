...
import Modelo1 from "../../Components/Form/modelo1";

const { Option } = Select;


const ListAbstractType2 = () => {
    ...
    const getResumes = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/user/${JSON.parse(sessionStorage.getItem('user')).user}/author/resume/clinical/`, {
                headers: {
                    Authorization: `Token ${sessionStorage.getItem('token')}`
                }
            });
    ...
    const handleRemoveReview = (record) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/api/resume/clinical/${record.id}/`, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
    ...
    const goEdit = (record) => {
        try {
            ...
            setSelected({
                ...
                resumo: record.resume,
                discussao: record.discussion,
    ...
    const submit = () => {
        ...
        let data = {
            ...
            introduction: selected.introducao, // form.getFieldValue('introducao'),
            ethic: selected.ethic,
            resume: selected.resumo, // form.getFieldValue('metodos'),
            discussion: selected.discussao, // form.getFieldValue('resultados'),
            conclusion: selected.conclusao, // form.getFieldValue('conclusao'),
            keyword: keywords,
            presentation: selected.presentation, // form.getFieldValue('tipoApresentacao'),
            category: selected.category, // form.getFieldValue('categoria'),
            subcategory: selected.subcategory, // form.getFieldValue('subCategoria'),
            author: JSON.parse(sessionStorage.getItem('user')).user,
            coauthor: selected.coauthor,
        }
        console.log(data)

        axios.put(`${process.env.REACT_APP_API_URL}/api/resume/clinical/${selected.id}/`, data, {
            headers: {
                'Authorization': `Token ${sessionStorage.getItem('token')}`
            }
        }).then(response => {
                    ...
                    <div className='w-full'>
                        <div className='w-full flex flex-row justify-between items-center mb-0'>
                            ...
                            <div className='hidden'>
                                <ComponentToPrint ref={printRef}>
                                    <Modelo1 abstract={selected} /> {/* passa a linha selecionada como prop */}
                                </ComponentToPrint>
                            </div>
                        </div>
                        <div className='w-full flex flex-col gap-2'>
                            ...
                            <div className='w-full'>
                                ...
                                <p className='text-sm rounded-md' style={{border: 'none'}}>{selected.resume}</p>
                                {selected.reviews?.map((item, index) => (
                                    <div key={index} className='w-full flex flex-col gap-2'>
                                        <p className='text-sm mb-2 flex items-center'>
                                            <FaRegComment className='mr-2'/>
                                            : {item.resume || "---Não Revisto---"}
                                        </p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className='w-full'>
                                ...
                                <p className='text-sm rounded-md' style={{border: 'none'}}>{selected.discussion}</p>
                                {selected.reviews?.map((item, index) => (
                                    <div key={index} className='w-full flex flex-col gap-2'>
                                        <p className='text-sm mb-2 flex items-center'>
                                            <FaRegComment className='mr-2'/>
                                            : {item.discussion || "---Não Revisto---"}
                                        </p>
                                    </div>
                                ))}
                            </div>
                ...
                >
                    <div className='w-full'>
                        ...
                        >
                            ...
                            >
                                Possui {countWords(selected.introducao) + countWords(selected.resumo) + countWords(selected.discussao) + countWords(selected.conclusao)}/350 Palavra(s) para preencher o corpo do resumo</p>
                            ...
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                    >
                                        <Input.TextArea placeholder='Insira o Resumo do caso (identificação, resumo da história clínica, exames complementares e/ou intervenções, diagnóstico final)'
                                                        value={selected.resumo}
                                                        onChange={(e) => {
                                                            setSelected({ ...selected, resumo: e.target.value })
                                                            form.setFieldsValue({ resumo: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(selected.resumo)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                    >
                                        <Input.TextArea placeholder='Insira a Discussão'
                                                        value={selected.discussao}
                                                        onChange={(e) => {
                                                            setSelected({ ...selected, discussao: e.target.value })
                                                            form.setFieldsValue({ discussao: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(selected.discussao)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
                            <Row gutter={16}>
                                <Col span={24}>
                                    <Form.Item
                                        ...
                                        className='mb-2'
                                    >
                                        <Input.TextArea placeholder='Insira a conclusão'
                                                        value={selected.conclusao}
                                                        onChange={(e) => {
                                                            setSelected({ ...selected, conclusao: e.target.value })
                                                            form.setFieldsValue({ conclusao: e.target.value })
                                                        }} />
                                        <div className="flex flex-row justify-between w-full m-0">
                                            <p
                                                className='ml-auto mb-0'
                                            >{countWords(selected.conclusao)} palavra(s)</p>
                                        </div>
                                    </Form.Item>
                                </Col>
                            </Row>
...
export default ListAbstractType2;